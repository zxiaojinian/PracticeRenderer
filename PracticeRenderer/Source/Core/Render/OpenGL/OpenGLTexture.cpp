...
namespace PR
{
		...
		: m_TextureSpecification(specification)
	{
		...
		glTextureStorage2D(m_RendererID, 1, m_InternalFormat, m_TextureSpecification.Width, m_TextureSpecification.Height);
	...
	OpenGLTexture2D::OpenGLTexture2D(const TextureSpecification& specification, const std::string& path)
		: m_TextureSpecification(specification), m_Path(path)
	{
		...
		PR_ASSERT(data, "Failed to load image!");
		...
		PR_ASSERT(internalFormat & dataFormat, "Format not supported!");
	...
	void OpenGLTexture2D::SetData(void* data, uint32_t size)
	{
		...
		glTextureSubImage2D(m_RendererID, 0, 0, 0, m_TextureSpecification.Width, m_TextureSpecification.Height, m_DataFormat, GL_UNSIGNED_BYTE, data);
	}
